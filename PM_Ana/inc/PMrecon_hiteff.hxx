...
public:
  ...
  void clear(){
    ...
    isohit   =  -1;
    misshit  =  -1;
    memset(misspln,-1,sizeof(misspln));
...
int PLN,PLN2,CH,CL,CL2,CLHIT,CLHIT2,CLHIT3,CELL,CELL2,NEI,TRA,TRA2,TRACELL,TRACL,TRACL2,HIT,DIST,DIST2,DIST3,dummy,TMP,TMP2;
bool hitcl[2][18][16];
...
bool fTracking(int mod){
  ...
  for(int VIEW=0;VIEW<2;VIEW++){
	...
	TMP2=0;
	for(TMP=0;TMP<ntracell[VIEW][TRA]-1;TMP++){
	  if((plane[VIEW][TRA][TMP]-plane[VIEW][TRA][TMP+1])!=1){
	    track.misspln[TMP2]=plane[VIEW][TRA][TMP+1]-1;
	    TMP2++;
	  }
	}
	track.misshit = TMP2;
