...
private:
  ...
  int                        mu_pln[2]; // most upstream pln
  int                       md_pln2[2]; // next most downstream pln
  vector<pair<int, int> > trk_point[2]; // <plane, ch> **[view]
  int                         n_trk[2];
  int                          n_match;

  vector<vector<pair<int, int> > > vec_trk_point[2]; // <plane, ch> **[view]

  vector<int>                      vec_mu_pln[2];
  vector<bool>                     vec_uvetoistrk[2];
  vector<bool>                     vec_ftplistrk[2];
  bool this_3TPL_is_already_found(int v, int i,int j,int k){
    int ns = vec_trk_point[v].size();
    bool flag=false;
    for(int is=0; is<ns; is++){
#ifdef DEBUG111206
      cout << "---- view "<< v << "  --------------------" << endl;
      cout << vec_trk_point[v][is][0].second << "\t" << i << endl;
      cout << vec_trk_point[v][is][1].second << "\t" << j << endl;
      cout << vec_trk_point[v][is][2].second << "\t" << k << endl;
      cout << "--------------------------" << endl;
#endif

      if( vec_trk_point[v][is][0].second == i && 
	  vec_trk_point[v][is][1].second == j && 
	  vec_trk_point[v][is][2].second == k ){
