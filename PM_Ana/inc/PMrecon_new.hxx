...
int view,pln[2][602],ch[2][602],pe[2][602],hit[2],hitnum[2][18][32];
...
bool vetowtracking[2][500],edgewtracking[2][500];
float ang[2];
  ...
  for(int VIEW=0;VIEW<2;VIEW++){
	...
	for(CELL=0;CELL<ncell[VIEW][PLN-DIST][DIST];CELL++){
	  for(DIST2=0;DIST2<2;DIST2++){
		...
		neib=false;
		for(CLHIT=0;CLHIT<numcl[VIEW][PLN-DIST][cellu[VIEW][PLN-DIST][CELL][DIST]];CLHIT++){
		  for(CLHIT2=0;CLHIT2<numcl[VIEW][PLN+1][celld[VIEW][PLN-DIST][CELL][DIST]];CLHIT2++){
		    for(CLHIT3=0;CLHIT3<numcl[VIEW][PLN+2+DIST2][celld[VIEW][PLN+1][CELL2][DIST2]];CLHIT3++){
		      
		      
		      x[0]=zposi(mod,VIEW,PLN-DIST);
		      x[1]=zposi(mod,VIEW,PLN+1);
		      x[2]=zposi(mod,VIEW,PLN+2+DIST2);
		      y[0]=xyposi(mod,PLN-DIST,clchi[VIEW][PLN-DIST][cellu[VIEW][PLN-DIST][CELL][DIST]]+CLHIT);
		      y[1]=xyposi(mod,PLN+1,clchi[VIEW][PLN+1][celld[VIEW][PLN-DIST][CELL][DIST]]+CLHIT2);
		      y[2]=xyposi(mod,PLN+2+DIST2,clchi[VIEW][PLN+2+DIST2][celld[VIEW][PLN+1][CELL2][DIST2]]+CLHIT3);
		...
		for(DIST2=0;DIST2<2;DIST2++){
		  ...
		  for(NEI=0;NEI<nnei[VIEW][PLN2][DIST2][DIST3];NEI++){
		    if(neid[VIEW][PLN2][NEI][DIST2][DIST3]==track_cell[VIEW][ntrack[VIEW]][ntracell[VIEW][ntrack[VIEW]]-1]&&
		       value[VIEW][PLN2-DIST2][neiu[VIEW][PLN2][NEI][DIST2][DIST3]][DIST2]+1==value[VIEW][PLN2+1][neid[VIEW][PLN2][NEI][DIST2][DIST3]][DIST3]){
		      if(!dwst){
			
			x[0]=zposi(mod,VIEW,PLN2-DIST2);
			x[1]=zposi(mod,VIEW,PLN2+1);
			x[2]=zposi(mod,VIEW,PLN2+2+DIST3);
			
			y[0]=clcenter[VIEW][PLN2-DIST2][cellu[VIEW][PLN2-DIST2][track_cell[VIEW][ntrack[VIEW]][ntracell[VIEW][ntrack[VIEW]]]][DIST2]];
			y[1]=clcenter[VIEW][PLN2+1][celld[VIEW][PLN2-DIST2][track_cell[VIEW][ntrack[VIEW]][ntracell[VIEW][ntrack[VIEW]]]][DIST2]];
			y[2]=clcenter[VIEW][PLN2+2+DIST3][celld[VIEW][PLN2+1][track_cell[VIEW][ntrack[VIEW]][ntracell[VIEW][ntrack[VIEW]]-1]][DIST3]];
			
			
			TGraph *graph1=new TGraph(3,x,y);
			TF1 *f1=new TF1("f1","[0]+[1]*x");
			f1->SetParameters(y[0]-x[0]*(y[2]-y[0])/(x[2]-x[0]),(y[2]-y[0])/(x[2]-x[0]));
			graph1->Fit("f1","Q");
			chi2=f1->GetChisquare();
			graph1->Delete();
			f1->Delete();
			
			y[0]=clcenter[VIEW][PLN2-DIST2][cellu[VIEW][PLN2-DIST2][neiu[VIEW][PLN2][NEI][DIST2][DIST3]][DIST2]];
	
			TGraph *graph2=new TGraph(3,x,y);
			TF1 *f2=new TF1("f2","[0]+[1]*x");
			f2->SetParameters(y[0]-x[0]*(y[2]-y[0])/(x[2]-x[0]),(y[2]-y[0])/(x[2]-x[0]));
			graph2->Fit("f2","Q");
			tmp=f2->GetChisquare();
			graph2->Delete();
			f2->Delete();
			...
			if((tmp>chi2||upstpe2<4.5)&&upstpe1>=4.5)continue;
    ...
    for(TRA=0;TRA<ntrack[VIEW];TRA++){
      plane[VIEW][TRA][0]=track_pln[VIEW][TRA][0]+track_dist[VIEW][TRA][0]+1;
      clus[VIEW][TRA][0]=celld[VIEW][track_pln[VIEW][TRA][0]][track_cell[VIEW][TRA][0]][track_dist[VIEW][TRA][0]];
      XX[VIEW][TRA][0]=zposi(mod,VIEW,plane[VIEW][TRA][0]);
      YY[VIEW][TRA][0]=clcenter[VIEW][plane[VIEW][TRA][0]][clus[VIEW][TRA][0]];
      trape[VIEW][TRA]+=clpe[VIEW][plane[VIEW][TRA][0]][clus[VIEW][TRA][0]];
      
      for(TRACELL=0;TRACELL<ntracell[VIEW][TRA];TRACELL++){
	
	plane[VIEW][TRA][TRACELL+1]=track_pln[VIEW][TRA][TRACELL];
	clus[VIEW][TRA][TRACELL+1]=cellu[VIEW][track_pln[VIEW][TRA][TRACELL]][track_cell[VIEW][TRA][TRACELL]][track_dist[VIEW][TRA][TRACELL]];
	XX[VIEW][TRA][TRACELL+1]=zposi(mod,VIEW,plane[VIEW][TRA][TRACELL+1]);
	YY[VIEW][TRA][TRACELL+1]=clcenter[VIEW][plane[VIEW][TRA][TRACELL+1]][clus[VIEW][TRA][TRACELL+1]];
	trape[VIEW][TRA]+=clpe[VIEW][plane[VIEW][TRA][TRACELL+1]][clus[VIEW][TRA][TRACELL+1]];
      }
      ntracl[VIEW][TRA]=ntracell[VIEW][TRA]+1;
    }
	...
	continue;
      ...
      bool otrack=false;
      while(1){
	TGraph *g=new TGraph(ntracell[VIEW][TRA2]+1,XX[VIEW][TRA2],YY[VIEW][TRA2]);
	TF1 *f=new TF1("f","[0]+[1]*x");
	f->SetParameters(YY[VIEW][TRA2][0]-XX[VIEW][TRA2][0]*(YY[VIEW][TRA2][ntracell[VIEW][TRA2]]-YY[VIEW][TRA2][0])/(XX[VIEW][TRA2][ntracell[VIEW][TRA2]]-XX[VIEW][TRA2][0]),(YY[VIEW][TRA2][ntracell[VIEW][TRA2]]-YY[VIEW][TRA2][0])/(XX[VIEW][TRA2][ntracell[VIEW][TRA2]]-XX[VIEW][TRA2][0]));
	g->Fit("f","Q");
	TF1 *func=g->GetFunction("f");
	par[VIEW][TRA2][0]=func->GetParameter(0);
	par[VIEW][TRA2][1]=func->GetParameter(1);
	g->Delete();
	f->Delete();
	chi2=func->GetChisquare();
	if(chi2/(ntracell[VIEW][TRA2]-1)<1000)break;
	else{
	  ntracell[VIEW][TRA2]--;
	  ntracl[VIEW][TRA2]--;
	}

	if(TRA!=(ntrack[VIEW]-1)&&ntracl[VIEW][TRA2]<ntracl[VIEW][rank[VIEW][TRA+1]]){
	  otrack=true;
	  break;
	}
	...
	continue;
      ...
      for(TRACL=0;TRACL<ntracl[VIEW][TRA2];TRACL++){
	if(hitcl[VIEW][plane[VIEW][TRA2][TRACL]][clus[VIEW][TRA2][TRACL]]==true)ovcl[VIEW][plane[VIEW][TRA2][TRACL]][clus[VIEW][TRA2][TRACL]]=true;
...
/*
bool fAnalysis(IngridEventSummary* evt,int Scyc,int Ncyc){
  for( int cyc=Scyc; cyc<Ncyc; cyc++ ){//### Cycle Loop
    for( int view=0; view<2; view++ ){//## VIEW
      int npmrecon = evt -> NPMModRecons(16, cyc, view);
      for( int mod=0; mod<15; mod++ ){//### Module Loop
	int ningrecon = evt -> NPMModRecons(mod, cyc, view);
	for( int ipm=0; ipm<npmrecon; ipm++ ){//## PM track
	  for( int iing=0; iing<ningrecon; iing++ ){//## INGRID track
