...
TH1D *  LGGain[ nMod ][ nView ][ nPln ][ nCh ];
...
bool GetCalibConst(int irun, int isubrun){
  //TFile* fTCalib   = new TFile( Form("/export/scraid2/data/bquilain/calib_root_file/ingrid_%08d_%04d_Calib00.root", irun, isubrun),"read" );
  TFile* fTCalib   = new TFile( Form("/home/bquilain/calib_root_file/ingrid_%08d_%04d_Calib00.root", irun, isubrun),"read" );
  ...
  for(int imod=0; imod<nMod; imod++){
	...
	for(int ich=0; ich<nCh ; ich++){
	  if( GoodCh[imod][iview][ipln][ich] ){
	    ...
	    //?
	    //if(imod==16) cout<<HighGain[imod][iview][ipln][ich]<<endl;
	    //if(imod==3) 
#ifdef DEBUG
	    if(imod==16 && ich>7 && ich<24) cout<<"Pedestal value="<<HighPed[imod][iview][ipln][ich]<<endl;
#endif
 ...
 for(int imod=0; imod<nMod; imod++){
	...
	for(int ich=0; ich<nCh ; ich++){
	  if( GoodCh[imod][iview][ipln][ich] ){
	    ...
	    double Hi_CI_Cal=Hi_Fit[imod][iview][ipln][ich]->Eval(gain[imod][iview][ipln][ich]);//ci value for ADC=gain
...
#endif	
	    Gain_Cal[imod][iview][ipln][ich]=Hi_CI_Cal/gain[imod][iview][ipln][ich];//Slope of the CI vs ADC
/*/(High_ADC_Cal[imod][iview][ipln][ich]-ped[imod][iview][ipln][ich])*/
	...
	for(int ich=0; ich<nCh ; ich++){
	  if( GoodCh[imod][iview][ipln][ich] ){
	    ...
	    sprintf(LGNormLineName,"LGNormLine%02d%d%02d%02d",imod,iview,ipln,ich);
            //LGGain[imod][iview][ipln][ich] = new TH1D(LGGainName,"Gain",1000,0,1000);
            //LGNorm[imod][iview][ipln][ich] = new TH1D(LGNormName,"Norm",1000,0,1000);
	    ...
	    for(int iadc=1;iadc<=NStep;iadc++){
	      ...
	      if(CIValue==0) gain_correction[imod][iview][ipln][ich]=Gain_Cal[imod][iview][ipln][ich]/(Hi_Fit[imod][iview][ipln][ich]->Eval(699)/699);//Gain cal gives the conversion factor betweeen ADC value at gain value and CI value. Here, we just have the valeu CI_factor_gain/CI_factor_ADC
	      else    gain_correction[imod][iview][ipln][ich]=Gain_Cal[imod][iview][ipln][ich]/(CIValue/(ADCNumber));//R1/R2            
	      gain_corrected[imod][iview][ipln][ich]=gain[imod][iview][ipln][ich]* gain_correction[imod][iview][ipln][ich];//gain*gain_correction=variation of the gain value due to ADC non linearity
...
int main(int argc,char *argv[]){
  ...
  TH1D * LoGainDistri = new TH1D("LoGainDistri","Low Gain",100,0,5);
  TH1D * HiGainDistri = new TH1D("HiGainDistri","Hi Gain",100,5,15);

  TH1D * hCI2PE = new TH1D("hCI2PE","CI2PE value for different channels",100,0.05,0.15);
  ...
  */
	...
	//if(mod==16) cout<<ninghit<<endl;
	  ...
	  if( GoodCh[mod][view][pln][ch] ){//forcement assure car on parcourt les évenements
		...
		//if(mod==16){
	      ...
	      Lope=Lo_Fit[mod][view][pln][ch]->Eval(inghitsum->loadc - loped[mod][view][pln][ch])*CI2PE[mod][view][pln][ch];
	      ...
	      if((inghitsum->adc - ped[mod][view][pln][ch])<700) Hipe=Hi_Fit[mod][view][pln][ch]->Eval(inghitsum->adc - ped[mod][view][pln][ch])*CI2PE[mod][view][pln][ch];
	      else Hipe=Hi_Fit[mod][view][pln][ch]->Eval(699)*CI2PE[mod][view][pln][ch];
		...
		nd->Fill(NDChan[mod][view][pln][ch]);
	      ...
	      double LGexp=1/(Lo_Fit[mod][view][pln][ch]->Eval(1)*CI2PE[mod][view][pln][ch]);
	      LoGainDistri->Fill(LGexp);
	      HiGainDistri->Fill(gain[mod][view][pln][ch]);
	      //cout<<"Logain is="<<LGexp<<endl;
	      ...
	      cout<<"High Gain="<<gain[mod][view][pln][ch]<<"    "<<"Lo Gain="<<logain[mod][view][pln][ch]<<"    "<<CI2PE[mod][view][pln][ch]<<endl;
	      cout<<"Charge Injected ="<<Hi_Fit[mod][view][pln][ch]->Eval(inghitsum->adc - ped[mod][view][pln][ch])<<"  ,Low= "<<Hi_Fit[mod][view][pln][ch]->Eval(inghitsum->loadc - loped[mod][view][pln][ch])<<endl;
		cout<<"Old PE Value in HG="<<HipeOld<<" ,New PE Value="<<Hipe<<" ||||  Old PE Value in LG="<<LopeOld<<" , New PE Value="<<Lope<<endl<<endl;
#endif
	      }
	      ...
	      Evt->Fill(Hipe);
	      if(Lope!=0) HiLo->Fill(Lope,Hipe/Lope);
	      if(Lope!=0) LoHi->Fill(Hipe,Hipe/Lope);
	      NormMean->Fill((Hipe+Lope)/2);
	      if(Lope!=0) HiLoMean->Fill((Hipe+Lope)/2,Hipe/Lope);
	      NormMeanOld->Fill((HipeOld+LopeOld)/2);
	      if(LopeOld!=0) HiLoMeanOld->Fill((HipeOld+LopeOld)/2,HipeOld/LopeOld);
	      LoHipe->Fill(Lope,Hipe);
	      hCI2PE->Fill(CI2PE[mod][view][pln][ch]);
	  ...
	  // inghitsum -> pe   = Hipe;
	 inghitsum -> pe   = (inghitsum->adc - ped[mod][view][pln][ch])/gain[mod][view][pln][ch];
	  //cout<<inghitsum -> pe<<endl;
	  //##### Conversion from ADC to #p.e.(Logain)#######
	  //#################################################
	  //cout<<"Conversion from ADC to #p.e.(Logain)"<<endl;
	  //inghitsum -> lope = Lope;
	  inghitsum -> pe   =(inghitsum->loadc - loped[mod][view][pln][ch])/logain[mod][view][pln][ch];
  ...
  hCI2PE->SaveAs("Calib_Conv/hCI2PE.root");

  HiGainDistri->SaveAs("Calib/HiGainDistri.root");
  LoGainDistri->SaveAs("Calib/LoGainDistri.root");
  wtree -> Write();
  wfile -> Write();
  wfile -> Close();
  //app.Run();
