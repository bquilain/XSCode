  ...
  */
  ...
  for(int ievt = 0; ievt < nevt; ++ievt){
	...
	cout<<"Module:"<<mod<<"   , Cycle:"<<cyc<<endl;
	  ...
	  //charger la fonction: 
	  double CI_ADC=Hi_Fit[mod][view][pln][ch]->Eval(inghitsum->adc);
	  cout<<CI_ADC<<endl;
	  cout<<"Gain was ="<<gain[mod][view][pln][ch]<<"  , Charge Injected is = "<<CI_ADC<<"  ,  ADC equivalent is = "<<inghitsum->adc<<endl;
	  gain_correction[mod][view][pln][ch]=(CI_ADC/inghitsum->adc)/Gain_Cal[mod][view][pln][ch];
	  gain[mod][view][pln][ch]*=gain_correction[mod][view][pln][ch];
	  cout<<"Gain corrected is now = "<<gain[mod][view][pln][ch]<<endl;
	  /*
	  double CI_ADC_Lo=Lo_Fit[mod][view][pln][ch]->Eval(inghitsum->loadc);
	  cout<<CI_ADC_Lo<<endl;
	  cout<<"Lo Gain was ="<<logain[mod][view][pln][ch]<<endl;
	  logain_correction[mod][view][pln][ch]=(Lo_Fit[mod][view][pln][ch]->Eval(inghitsum->loadc)/inghitsum->loadc)/Gain_Cal_Lo[mod][view][pln][ch];
	  logain[mod][view][pln][ch]*=logain_correction[mod][view][pln][ch];
	  cout<<"Gain corrected is now = "<<logain[mod][view][pln][ch]<<endl;*/
