	...
	for(int ich=0; ich<nCh ; ich++){
	  if( GoodCh[imod][iview][ipln][ich] ){
	    if(TMath::Abs(gain[imod][iview][ipln][ich]-10.)<5. && (Gain_Cal[imod][iview][ipln][ich] <1.5)){
	      LGCal[imod][iview][ipln][ich]->Sumw2();
	      LGCal[imod][iview][ipln][ich]->Divide(LGCalNorm[imod][iview][ipln][ich]);
	      if(NCal2[imod][iview][ipln][ich]==0) {
		cout<<"No Count in this channel under 15p.e, Chan = "<<NDChan[imod][iview][ipln][ich]<<endl;
		logain_cal[imod][iview][ipln][ich]=1;
		SlopeCal[imod][iview][ipln][ich]=10;
	      }
	      else if(NCal[imod][iview][ipln][ich]==0){
		logain_cal[imod][iview][ipln][ich]=logain_cal2[imod][iview][ipln][ich]/NCal2[imod][iview][ipln][ich];
		SlopeCal[imod][iview][ipln][ich]=SlopeCal2[imod][iview][ipln][ich]/NCal2[imod][iview][ipln][ich];
	      }
	      else{
		logain_cal[imod][iview][ipln][ich]/=NCal[imod][iview][ipln][ich];
		SlopeCal[imod][iview][ipln][ich]/=NCal[imod][iview][ipln][ich];
	      }
	    ...
	    //prendre le(s) bin(s) de calibration: regarder l'histo d'abord. Prendre peut etre les 10 premiers (en gros de 0 a 10 p.e )
	      ...
	      Gain_Cal_Lo=SlopeCal[imod][iview][ipln][ich];
	      LoGainCal=logain_cal[imod][iview][ipln][ich];
	      
	      for(int ilin=0;ilin<NBins;ilin++){
		
		CI_ADC_Lo=Lo_Fit[imod][iview][ipln][ich]->Eval(ilin);
		if(CI_ADC_Lo==0) logain_corrected[imod][iview][ipln][ich]=LoGainCal;//case over CI calibration
		else logain_corrected[imod][iview][ipln][ich]=LoGainCal*Gain_Cal_Lo/(CI_ADC_Lo/(ilin));
...
int main(int argc,char *argv[]){
  ...
  TH1D * Lpe10 = new TH1D("Lpe10","p.e in LG Chan",100,0,100);
  TH1D * OldHpe10 = new TH1D("Hpe10Old","p.e in HG Chan, before tuning",100,0,100);
  TH1D * OldLpe10 = new TH1D("Lpe10Old","p.e in LG Chan, before tuning",100,0,100);
  TH1D * PENew = new TH1D("PENew","#p.e",100,0,100);
  TH1D * PEOld = new TH1D("PEOld","#p.e",100,0,100);
  ...
  */
  ...
  for(int ievt = 0; ievt < nevt; ++ievt){
    if(ievt%500==0)cout<<"Run#"<<run_number<<"\tcalib event:"<<ievt<<endl;
    summary -> Clear();
    wsummary-> Clear();
    tree    -> GetEntry(ievt);
    for(int mod=0; mod<nMod; mod++){
	...
	cout<<"Module:"<<mod<<"   , Cycle:"<<cyc<<endl;                                                                                              
	    ...
	    for(int i=0; i<ninghit; i++){
	      ...
	      int ch   = inghitsum -> ch;
	      	      if( GoodCh[mod][view][pln][ch] ){
#ifdef DEBUG
	      cout<<"Plan = "<<pln<<" , Channel = "<<ch<<" , View = "<<view<<endl;
#endif
	      if(TMath::Abs(gain[mod][view][pln][ch]-10.)<7. && (Gain_Cal[mod][view][pln][ch] <1.7)){
		HADC=inghitsum->adc-ped[mod][view][pln][ch];
		//cout<<HADC<<endl;
		int Lin=(int) HADC/10;
		//	cout<<Lin<<endl;
		HGpe=(HADC)/(HGGainLine[mod][view][pln][ch]->GetBinContent(Lin));
		//cout<<HGpe<<endl;
		LADC=inghitsum->loadc-loped[mod][view][pln][ch];
		//cout<<LADC<<endl;
		
		//if(LADC>0 && LADC<10 && HGpe>0 && HGpe<10000){
		if(HGpe>0 && HGpe<15 && LADC>0){
		  if((HGpe/LADC)!=(HGpe/LADC)) {
		    cout<<"Module:"<<mod<<" , Plan = "<<pln<<" , Channel = "<<ch<<" , View = "<<view<<endl;
		    cout<<"HGpe="<<HGpe<<" , LoaDC="<<LADC<<endl<<endl;
		    continue;
		  }
		  if(HGpe>2 && HGpe<10 && LADC>0){
		    logain_cal[mod][view][pln][ch]+= HGpe/LADC;
		    SlopeCal[mod][view][pln][ch]+=Lo_Fit[mod][view][pln][ch]->Eval(LADC)/LADC;
		    NCal[mod][view][pln][ch]++;
		  }
		  logain_cal2[mod][view][pln][ch]+= HGpe/LADC;
		  SlopeCal2[mod][view][pln][ch]+=Lo_Fit[mod][view][pln][ch]->Eval(LADC)/LADC;
		  NCal2[mod][view][pln][ch]++;
		  
		//		if(logain_cal>1000) cout<<"HGpe="<<HGpe<<" ,LADC="<<LADC<<endl;
		//cout<<logain_cal<<endl;
		//le définir avec un binning en Line, mais c'est en gros, le binning de LoADC...
		//LGCal[mod][view][pln][ch]->Fill(LADC,logain_cal);
		//LGCalNorm[mod][view][pln][ch]->Fill(LADC);
		//TotLGCal->Fill(LADC,logain_cal);
		//TotLGCalNorm->Fill(LADC);
	      }
	      }
	      }
	    }
  ...
  //TotLGCal->Sumw2();
  //TotLGCal->Divide(TotLGCalNorm);
  //TotLGCal->SaveAs("Calib/Cal.root");
 ...
 //logain_correction[mod][view][pln][ch]=(Lo_Fit[mod][view][pln][ch]->Eval(inghitsum->loadc - loped[mod][view][pln][ch])/(inghitsum->loadc - loped[mod][view][pln][ch]))/Gain_Cal_Lo[mod][view][pln][ch];
	  ...
	  */
	  ...
	  if( GoodCh[mod][view][pln][ch] ){//forcement assure car on parcourt les évenements
	    if(TMath::Abs(gain[mod][view][pln][ch]-10.)<5. && (Gain_Cal[mod][view][pln][ch] <1.5)){
	      ...
	      double Lope= (inghitsum->loadc - loped[mod][view][pln][ch])/LGCorr;                     
	      ...
	      double Hipe= (inghitsum->adc - ped[mod][view][pln][ch])/HGCorr;
  ...
  nd->SaveAs("Calib/nd280.root");
  hGainH->Scale(1./hGainH->Integral());
  hGainH->SaveAs("Calib/hGainH.root");
  hSlopeH->Scale(1./hSlopeH->Integral());
  hSlopeH->SaveAs("Calib/hSlopeH.root");
  hGainL->Scale(1./hGainL->Integral());
  hGainL->SaveAs("Calib/hGainL.root");
  hSlopeL->Scale(1./hSlopeL->Integral());
  hSlopeL->SaveAs("Calib/hSlopeL.root");
  bGainH->Scale(1./bGainH->Integral());
  bGainH->SaveAs("Calib/bGainH.root");
  bSlopeH->Scale(1./bSlopeH->Integral());
  bSlopeH->SaveAs("Calib/bSlopeH.root");
  bGainL->Scale(1./bGainL->Integral());
  bGainL->SaveAs("Calib/bGainL.root");
  bSlopeL->Scale(1./bSlopeL->Integral());
  bSlopeL->SaveAs("Calib/bSlopeL.root");
  LoHi->Sumw2();                        
  LoHi->Divide(Evt);                                                                                 
  LoHi->SaveAs("Calib/lohi.root"); 
  HiLo->Sumw2();
  HiLo->Divide(Evt);
  HiLo->SaveAs("Calib/hilo.root");
  HiLoMean->Sumw2();
  HiLoMean->Divide(NormMean);
  HiLoMean->SaveAs("Calib/hilomean.root");
  HiLoMeanOld->Sumw2();
  HiLoMeanOld->Divide(NormMeanOld);
  HiLoMeanOld->SaveAs("Calib/hilomeanold.root");
  Hpe10->SaveAs("Calib/highperepartition.root");
  Lpe10->SaveAs("Calib/Lowperepartition.root");
  OldHpe10->SaveAs("Calib/OldHighperepartition.root");
  OldLpe10->SaveAs("Calib/OldLowperepartition.root");
  PENew->SaveAs("Calib/PENew.root");
  PEOld->SaveAs("Calib/PEOld.root");
  wtree -> Write();
  wfile -> Write();
  wfile -> Close();
  //app.Run();
