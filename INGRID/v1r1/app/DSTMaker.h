...
void Book(Int_t run, Int_t subrun){
  char buff[300];
  if(!cosmic&&!PModule)
    sprintf(buff,"/home/bquilain/Ingrid_Process/dst/ingrid_%08d_%04d.root",run,subrun);
  else if(cosmic&&!PModule)
    sprintf(buff,"/home/bquilain/Ingrid_Process/cosmic/ingrid_%08d_%04d.root",run,subrun);
  else if(!cosmic&&PModule)
    sprintf(buff,"/home/bquilain/Ingrid_Process/PM/ingrid_%08d_%04d.root",run,subrun);
  else if(cosmic&&PModule)
    sprintf(buff,"/home/bquilain/Ingrid_Process/PM_cosmic/ingrid_%08d_%04d.root",run,subrun);
...
void Read(ND::TND280RawEvent* re){
  ...
  */
  ...
  if(TrgId==cAnaTrg){
	...
	//	cout<<mod<<endl;
	if(!read)continue;
	cout<<"Module = "<<mod<<"  ,Plane is = "<<plane<<"  ,Channel = "<<ch<<"  ,View = "<<view<<endl;
	cout<<"RMM = "<<rmm<<"  ,TFB = "<<tfb<<"  ,trip = "<<trip<<"  , Ch in trip = "<<trip_ch<<"  ,ND280 Ch = "<<100*(rmm*48+tfb)+16*trip+trip_ch<<endl;
	cout<<mod*1000+plane*48+ch*2+view<<"   "<<100*(rmm*48+tfb)+16*trip+trip_ch<<endl<<endl;
