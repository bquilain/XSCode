...
int Neut::NtupleReadEvent(SecondaryVector &Secondary, float* direction)
{
  ...
  std::cout << "vertex: ";
  for (int k=0;k<3;k++)
    std::cout << ((ParentVector) Vector.Neutrino.ParentDecay).Vertex[k] << " ";
  std::cout << "\ndirection: ";
  for (int k=0;k<3;k++)
...
*/
  ...
  for(int i=0;i<3;i++){
    //std::cout << " GPOS["<<i<<"]: " << Vector.Neutrino.primary_particle.gpos0[i] << "\n";
    //std::cout << " Parent GPOS["<<i<<"]: " << Vector.Neutrino.ParentProduction.Vertex[i] << "\n";
    //std::cout << " GPOS[0]: " << Vector.Neutrino.primary_particle.gpos0[i] << "\n";
