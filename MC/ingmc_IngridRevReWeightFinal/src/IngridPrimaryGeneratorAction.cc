...
const double scibar_width = 1.3049; //(cm) (total mass of scibar type)
//const double width_ingrid =1.0; //INGRID type (design value)
const double width_ingrid =0.982; //INGRID type (measured value)
//const double width_scibar =1.3; //SciBar type (design value)
const double width_scibar =1.274; //SciBar type (measured value)
const double offset_ingrid =0.009;
const double offset_scibar =0.013;
...
IngridPrimaryGeneratorAction(Neut *neut0,IngridRunAction* rac, IngridEventAction* evt,int nd,int flavor0, bool reduced_ingrid=false,int ingrid_mod=-1)
  :runaction(rac)
{
  eventaction = evt;
  neut_fe = neut0;
...
void IngridPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  // Define neutrino interaction vertex
    ...
    //double PosX=-6.3; double PosY=2.7;//Run 1
    //double PosX=-0.2; double PosY=1.4;//Run 2
    // double PosX=8.35; double PosY=3.6;//Run 3
    //double PosX=1.9; double PosY=2.7;
...
#ifdef INT_FLOOR
      pos[2] = -120.-170.+340.*G4UniformRand();
#else
	...
	pos[2] = width_ingrid*(G4UniformRand());
        pos[2] = pos[2] + Pscinti_start + flayer*distance_pln+offset_ingrid;
      }
      else if( vertex_flag==1 ) {
        flayer = (int)(34*(G4UniformRand()));     // 0 < 34*rand < 34
        pos[2] = width_scibar*(G4UniformRand());
        pos[2] = pos[2] + Pscinti_start + distance_pln + diff + distance_first + flayer*distance_pln+offset_scibar;
      }
	...
	if(scitype==0){
	  pos[2] = width_scibar*(G4UniformRand());
	  pos[2] = pos[2] + Pscinti_start + 2*distance_pln + diff + distance_first + flayer*distance_pln*2+offset_scibar;
	}
	else{
	  pos[2] = width_ingrid*(G4UniformRand());
	  pos[2] = pos[2] + Pscinti_start + distance_pln + distance_first + flayer*distance_pln*2+offset_ingrid;
	}
      }
	...
	if(scitype==0){
	  pos[2] = width_scibar*(G4UniformRand());
	  pos[2] = pos[2] + Pscinti_start + distance_pln + diff + distance_first + flayer*distance_pln*2+offset_scibar;
	}
	else{
	  pos[2] = width_ingrid*(G4UniformRand());
	  pos[2] = pos[2] + Pscinti_start + 2*distance_pln + distance_first + flayer*distance_pln*2+offset_ingrid;
	}
      }
      else if( vertex_flag==4){
	flayer = (int)(34*(G4UniformRand()));     // 0 < 34*rand < 34
        pos[2] = width_ingrid*(G4UniformRand());
        pos[2] = pos[2] + Pscinti_start + distance_pln + distance_first + flayer*distance_pln+offset_ingrid;
      }
    ...
    */
    ...
    if( (fdid==3 && module_mode == HORIZONTAL) || (fdid==3 && module_mode == PROTON) ) {
#ifdef INT_FLOOR
      if( fabs(pos[0]) <= 450 &&
	  fabs(pos[1]) <= 70 ) {
	ID = 3;
	pos[1]=pos[1]-85.6-70.0;
	goto NEXTSTEP;
      }
#else
      for( int m=0;m<7;m++ ) {
    ...
    if( (fdid==4 && module_mode == VERTICAL) || (fdid==4 && module_mode == PROTON) ) {
      for( int m=0;m<7;m++ ) {
        if( fabs(pos[0]) <= 60 &&
