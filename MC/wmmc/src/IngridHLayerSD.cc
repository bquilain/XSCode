...
IngridHLayerSD::IngridHLayerSD(const G4String& name, G4double cbirks)
...
void IngridHLayerSD::EndOfEvent(G4HCofThisEvent* HCTE)
{
  ...
  for(G4int k=0;k<hlayerHitCollection->entries();k++) {
    ...
    mod = cHit->GetMod();
    view = cHit->GetView();
    pln  = cHit->GetPln();//2014/12/16
    ch   = cHit->GetCh();//2014/12/16

    //apply light collection
    //ingresp->ApplyLightCollection(&edep_tmp,mod,view,posinmod);
    ingresp->ApplyLightCollection(&edep_tmp,mod,view,pln,ch,posinmod);//2014/12/16

    //apply fiber attenuation
    ingresp->ApplyFiberResponse(&edep_tmp,&time_tmp,view,posinmod,mod);

    //convert edep -> p.e. &  p.e. statistic & cross-talk & after-pulse
    ingresp->ApplyMPPCResponse(edep_tmp,&pe,mod);

    //apply ADC responce
    //comment outed by koga 2016/7/27
