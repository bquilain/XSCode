...
////////////////////////////
{
  ...
  //
  p_theta_muons = new TH2D("pthetamuons","",100,0,5,100,-1,1);  
...
{ 
    ...
    if(this->produceDistributions){
      TFile * kinematic_distributions =new TFile("kinematic_distributions.root","recreate"); 
      G4cout<<"Write kinematic distributions..."<<G4endl;
      p_theta_muons->Write();
      p_theta_piPos->Write();
      p_theta_piNeg->Write();
      p_theta_protons->Write();
      kinematic_distributions->Close();
    }
    if(p_theta_muons) delete p_theta_muons;
    if(p_theta_piNeg) delete p_theta_piNeg;
