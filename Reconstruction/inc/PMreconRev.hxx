...
float pe[2][602]; float lope[2][602];
...
int hit_ch[2][500][200];
int hit_pln[2][500][200];
float hit_pe[2][500][200];
float hit_lope[2][500][200];
...
bool fTracking(int mod){
  ...
  for(int i=0; i<hitcls.size(); i++){
    ...
    lope[view][hit[view]]=hitcls[i].lope;
  ...
  for(int VIEW=0;VIEW<2;VIEW++){
	...
	//for(int TRACL=1;TRACL<ntracl[VIEW][TRA]-1;TRACL++){
    ...
    for(int TRA=0;TRA<ntrack[VIEW];TRA++){
      ...
      //if((mod!=16&&plane[VIEW][TRA][ntracl[VIEW][TRA]-1]==0)||(mod==16&&plane[VIEW][TRA][ntracl[VIEW][TRA]-1]<=1)){vetowtracking[VIEW][TRA]=true;}
      if(plane[VIEW][TRA][ntracl[VIEW][TRA]-1]==0) {vetowtracking[VIEW][TRA]=true;}
    ...
    */
    ...
    for(TRA=0;TRA<ntrack[VIEW];TRA++){
      if(ttrack[VIEW][TRA]){
        ...
        for(int TRACL=1;TRACL<ntracl[VIEW][TRA]-1;TRACL++){
          for(CLHIT=0;CLHIT<numcl[VIEW][plane[VIEW][TRA][TRACL]][clus[VIEW][TRA][TRACL]];CLHIT++){
            hit_ch[VIEW][TRA][TMP]=ch[VIEW][clhit[VIEW][plane[VIEW][TRA][TRACL]][clus[VIEW][TRA][TRACL]][CLHIT]];
            hit_pln[VIEW][TRA][TMP]=pln[VIEW][clhit[VIEW][plane[VIEW][TRA][TRACL]][clus[VIEW][TRA][TRACL]][CLHIT]];
            hit_pe[VIEW][TRA][TMP]=pe[VIEW][clhit[VIEW][plane[VIEW][TRA][TRACL]][clus[VIEW][TRA][TRACL]][CLHIT]];
            hit_lope[VIEW][TRA][TMP]=lope[VIEW][clhit[VIEW][plane[VIEW][TRA][TRACL]][clus[VIEW][TRA][TRACL]][CLHIT]];
            TMP++;
          }
        }
        hit_n[VIEW][TRA]=TMP;
    ...
    */
	...
	track.isohit.clear();
	/*
	track.hit_n = hit_n[VIEW][TRA];
        for(TMP=0;TMP<hit_n[VIEW][TRA];TMP++){
          track.hit_pln[TMP] = hit_pln[VIEW][TRA][TMP];
          track.hit_ch[TMP] = hit_ch[VIEW][TRA][TMP];
          track.hit_pe[TMP] = hit_pe[VIEW][TRA][TMP];
          track.hit_lope[TMP] = hit_lope[VIEW][TRA][TMP];
	  cout<<"LOPE="<<hit_lope[VIEW][TRA][TMP]<<endl;
        }*/
	for(TMP=0;TMP<hit_n[VIEW][TRA];TMP++){
	   track.hitid.push_back(hit_id[VIEW][TRA][TMP]);
	   track.isohit.push_back(isohit[VIEW][TRA][TMP]);
	   //cout<<"In *.h="<<hit_id[VIEW][TRA][TMP]<<endl;
	}
