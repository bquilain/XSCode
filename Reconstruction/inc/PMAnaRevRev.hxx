...
void addMuCL(vector<Hits> &allhit, float ang,int ipln, float intcpt, float slope){
  ...
  for(int k=0;k<allhit.size();k++){
    ...
    plnpe[allhit[k].pln]+=PE(allhit[k].pe,allhit[k].lope,allhit[k].mod,allhit[k].view,allhit[k].pln,allhit[k].ch)*fabs(cos(ang*3.14159265/180))/corr;
    if(allhit[k].pln>0&&allhit[k].ch>=8&&allhit[k].ch<24) stype[0][allhit[k].pln]++;
    else stype[1][allhit[k].pln]++;
    int L;
    if(allhit[k].view==0) L= intcpt+slope*zposi(allhit[k].mod,allhit[k].view,allhit[k].pln);
    else L= 1200-intcpt+slope*zposi(allhit[k].mod,allhit[k].view,allhit[k].pln);
    cout<<allhit[k].mod<<", pln="<<allhit[k].pln<<", ch="<<allhit[k].ch<<", pe="<<allhit[k].pe<<", pecorr="<<allhit[k].pe*cos(ang*3.14159265/180)/corr<<", PETot="<<PE(allhit[k].pe,allhit[k].lope,allhit[k].mod,allhit[k].view,allhit[k].pln,allhit[k].ch)*cos(ang*3.14159265/180)/corr<<", dx="<<1/cos(ang*3.14159265/180)<<", att="<<corr<<", L="<<L/10.<<", z="<<zposi(allhit[k].mod,allhit[k].view,allhit[k].pln)/10<<endl;
  }
  for(int i=0;i<18;i++){
    if(hitpln[i]){
...
void addIngHitMuCL(int incmod, int view, float ang,float intcpt, float slope,float intcpt2, float slope2){
  ...
  for(int pln=0;pln<=1;pln++){
	...
	plnpe[pln]+=ingnonrechits[incmod][view][pln][ch]*fabs(cos(ang*3.14159265/180))/corr;
      }
    }
  }
...
void fAddPE(vector<Hits> &allhit, float ang, float &totalpe, int &totalhit, int *trkpdg,int ipln, float intcpt, float slope){
  ...
  for(int k=0;k<allhit.size();k++){
    ...
    totalpe+=PE(allhit[k].pe,allhit[k].lope,allhit[k].mod,allhit[k].view,allhit[k].pln,allhit[k].ch)*fabs(cos(ang*3.14159265/180))/corr;
...
void fTrackMatch(Trk &trk, TrackPM &htrk, TrackPM &vtrk){
  ...
  if(!trk.ing_trk){
    ...
    trk.iron_range=vtrk.iron_pene/fabs(cos(trkang*3.14159265/180));
  }
  else{
    trk.iron_pene=htrk.iron_pene;
    //trk.iron_range=htrk.iron_pene/cos(trkang*3.14159265/180);
    trk.iron_range=htrk.iron_pene/fabs(cos(trkang*3.14159265/180));
  }
  
  if((vtrk.fpln-vtrk.ipln)>(htrk.fpln-htrk.ipln))
    //trk.sci_range=(vtrk.fpln-vtrk.ipln)/cos(trkang*3.14159265/180);
    trk.sci_range=(vtrk.fpln-vtrk.ipln)/fabs(cos(trkang*3.14159265/180));
  else
    //trk.sci_range=(htrk.fpln-htrk.ipln)/cos(trkang*3.14159265/180);
    trk.sci_range=(htrk.fpln-htrk.ipln)/fabs(cos(trkang*3.14159265/180));
