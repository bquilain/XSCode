...
#include "TrackMatching3D.hxx"
#include "Vertexing.hxx"
...
int main(int argc,char *argv[]){
  ...
  while ((c = getopt(argc, argv, "r:s:f:cado:i:k:m:n:")) != -1) {
    ...
    case 'i':
      ...
      break;
    case 'k':
      Criteria3D=atoi(optarg);
      break;
    case 'm':
      CriteriaZ=atoi(optarg);
      break;
    case 'n':
      CriteriaXY=atof(optarg);
      break;
    }
  }
  FileStat_t fs;
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	fIngAna(mod, Criteria3D);
	//cout<<"before Vertex Recons"<<endl;
	fIngVertex(mod, CriteriaZ, CriteriaXY);
