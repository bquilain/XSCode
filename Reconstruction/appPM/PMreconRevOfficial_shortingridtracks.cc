...
int main(int argc,char *argv[]){
  ...
  double FVCriteria=100;//cm, size of FV, always centered
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	NActivePlanes[mod][cyc]=0;NTracks[mod][cyc]=0;
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  if(mod==16 && recon->nactpln < ActivePlaneCriteria)continue;
	  else if(recon->nactpln < (ActivePlaneCriteria-1))continue;
	  recon->layerpe = fLayerpe(mod);
	  NActivePlanes[mod][cyc]=fNactpln(mod);
	  NTracks[mod][cyc]=0;
	  //if(recon->layerpe < 6.5)continue;
	  if(!fTracking(mod, VetoUpstreamCriteria, VetoEdgeCriteria, FVCriteria)) continue;
	  NTracks[mod][cyc]=alltrack.size();
	  //if(NActivePlanes[mod][cyc]!=0) cout<<"************************************************"<<endl;
	  //if(NActivePlanes[mod][cyc]!=0) cout<<NActivePlanes[mod][cyc]<<endl;
