...
int main(int argc,char *argv[]){
  ...
  //Hit                        hit;
  Track                        track;
  for(int ievt=Nini; ievt<nevt; ievt++){//boucle sur les évenements i.e sur les simvertex pour le MC?
	...
	htrack.clear();
	vtrack.clear();
	for( int view=0; view<2; view++ ){  //### View => regarde les traces 2D
	  //allhit.clear();
	  ...
	  for(int i=0; i<ningtrack; i++){
	    ...
	    if(view==0)//on crée une liste de traces 2D horiz et verticales
	      htrack.push_back(track);
	    else
	      vtrack.push_back(track);
	  }
	}//end of view = end of 2D track check

	fIngAna(mod);
	fIngVertex(mod);

	for(int i=0;i<ingtrack.size();i++){//boucles sur, non pas des traces 3D, mais sur un vertex (pls traces 3D matchées) 
	  ...
	  evt   -> AddBasicRecon( ingrecon );//added for each ingridtrack, for each module, each cycle. Peut il y avoir 2 ingridtrack issues du meme vertex mais mis dans 2 basicrecon différentes? Oui si les conditions dans fIngAnaVertex ne sont pas remplies (vertex assez proches) => va surement reconstruire souvent 2 vertex pour des inté très inelastiques. Cependant, dans la plupart des cas, quand on a plusieurs traces issues du meme vertex, on aura 1 BasRec avec pls Ntracks dedans. Et la plupart du temps (sauf les quelques inelastiques mentionnées), et bien 2 IngbasRec correspondent a 2 traces vraiment différentes: Dans MC, sachant qu'on a pas de Pile-up, c'est une inté très inelastique ou du bruit!
	}
