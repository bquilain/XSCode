...
int main(int argc,char *argv[]){
  ...
  Add_Bad_Channel();
#endif
  TFile * _GainVariation = new TFile("_GainVariation.root","READ");
  TH1D * GainVariation = (TH1D*) _GainVariation->Get("GainVariation");
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
    ...
    int Time=evt->spilltrg;
    int NBins=GainVariation->GetNbinsX();
    double TimeMin=GainVariation->GetBinLowEdge(1);
    double TimeMax=GainVariation->GetBinUpEdge(NBins);
    int TimeBin=(int) Time*((TimeMax-TimeMin)/NBins);
    double GainCorrection = GainVariation->GetBinContent(TimeBin);
	...
	for(int i=0; i<ninghit; i++){
	  ...
	  inghitsum ->pe = (inghitsum ->pe)*GainCorrection;
	  inghitsum ->lope = (inghitsum ->lope)*GainCorrection;
