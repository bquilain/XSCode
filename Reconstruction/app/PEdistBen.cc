...
int main(int argc,char *argv[]){
  ...
  ifstream pefile("/home/kikawa/PM_Ana/app/pe.txt");
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  for(int i=0;i<alltrack.size();i++){
	    ...
	    */
	    if(alltrack[i].veto|| alltrack[i].edge) continue;
	    hit_mod=mod;
	    hit_view=alltrack[i].view;
	    for(TMP=0;TMP<alltrack[i].hit_n;TMP++){
	      hit_pln=alltrack[i].hit_pln[TMP];
	      hit_ch=alltrack[i].hit_ch[TMP];
	      hit_pe=alltrack[i].hit_pe[TMP];//PE(alltrack[i].hit_pe[TMP],alltrack[i].hit_lope[TMP],hit_mod,hit_view,hit_pln,hit_ch)*corr;
	      wtree->Fill();
	    }
