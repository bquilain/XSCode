...
#include "TApplication.h"
#include "PMrecon_KS.hxx"
#include "PMAnaRev_KS_Systematics.hxx" //Kikawa-san study. Remettre aussi le mucl de trk commenté et vertex activity
//#include "PMAnaRev_BNJ.hxx" //Kikawa-san study
	...
	for(int t=0;t<pmtrack[i].trk.size();t++){
	  for(int ihit=0;ihit<pmtrack[i].trk[t].hit.size();ihit++){
		...
		for(int NHIT=0;NHIT<(recon->Nhits());NHIT++){
		  ...
		  //cout<<"For PMHit classic, xy="<<pmhit->xy<<", z="<<pmhit->z<<", channel="<<pmhit->ch<<", plane="<<pmhit->pln<<endl;
		  //cout<<"For PMHit from trk, xy="<<pmtrack[i].trk[t].hit[ihit].xy<<", z="<<pmtrack[i].trk[t].hit[ihit].z<<", channel="<<pmtrack[i].trk[t].hit[ihit].ch<<", plane="<<pmtrack[i].trk[t].hit[ihit].pln<<endl;
