...
int main(int argc,char *argv[]){
  ...
  //cout<<"CAREFUL: Information in hit->pe is a composition of low and high gain channel"<<endl;
  ...
  double FVCriteria=100;
  
  while ((c = getopt(argc, argv, "r:s:f:cado:i:k:m:n:")) != -1) {
    switch(c){
    case 'r':
    ...
    case 'i':
      ...
      break;
    case 'k':
      VetoUpstreamCriteria=atoi(optarg);
      break;
    case 'm':
      VetoEdgeCriteria=atof(optarg);
      break;
    case 'n':
      FVCriteria=atof(optarg);
      break;
    }
  }
  FileStat_t fs;
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	for(int i=0; i<ninghit; i++){
	  //Added by B.Quilain on 2013/04/22
	  //cout<<"before bquilain"<<endl;
	  //cout<<inghitsum->pe<<"   "<<inghitsum->lope<<endl;

	  /*  if((inghitsum -> pe)<100000 && (inghitsum -> lope)<100000){
	    //if( inghitsum -> pe < 20) inghitsum -> pe += 0; 
	    //else if(inghitsum -> pe < 40) inghitsum -> pe = (inghitsum -> pe + inghitsum -> lope )/2.;
	    //else inghitsum->pe=inghitsum->lope;
	    if( inghitsum -> pe < 50) inghitsum -> pe = inghitsum -> pe;
	    else inghitsum -> pe = inghitsum -> lope;
	  }*/
	  ...
	  //cout<<"Time = "<<inghitsum -> time<<"   , tdc = "<<inghitsum -> tdc<<endl;
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  if(!fTracking(mod, VetoUpstreamCriteria, VetoEdgeCriteria, FVCriteria))continue;
	  //fTracking(mod);
	  ...
	  for(int i=0;i<alltrack.size();i++){
	    ...
	    for(int NHIT=0;NHIT<alltrack[i].hitid.size();NHIT++){
	      ...
	      /************************Added by B.Quilain 2013/04/03************************/
	      recon-> HitId.push_back(alltrack[i].hitid[NHIT]);
	      recon->HitNum[NHIT]=alltrack[i].hitid[NHIT];
	      recon->nhits++;
	    }
	    //recon->hello=10;
	    //cout<<alltrack[i].hitid.size()<<endl;
	  ...
	  PMReconSummary * ben=new PMReconSummary();
	  for(int view=0;view<2;view++){
	  int npmrec=evt->NPMModRecons(mod,cyc,view);
	  //if(npmrec!=0) cout<<npmrec<<endl;
	  for(int rec=0;rec<npmrec;rec++){
	    ben=evt->GetPMModRecon( rec , mod, cyc, view);
	    //cout<<ben->hello<<endl;
	  }
	  }
