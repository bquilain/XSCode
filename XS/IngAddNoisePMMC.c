...
int main(int argc,char *argv[]){
  ...
  */
  ...
  cout<<"Noise rate="<<(NDUMMY_in)<<" for INGRID and "<<(NDUMMY_in*RatioHitPMINGRID)<<" for PM"<<endl;
  for(int ievt = 0; ievt < nevt; ++ievt){
    if(ievt%100==0)cout<<"\tadd noise event:"<<ievt<<endl;
	...
	//###################################################
	if(mod!=16) NDUMMY=NDUMMY_in;
	else NDUMMY=NDUMMY_in*RatioHitPMINGRID;
	...
	for(int idummyhit = 0; idummyhit < ndummyhit; idummyhit++){
	  ...
	  bool SameHit=false;
          for(int ihit2=0; ihit2 < summary->NIngridModHits(mod,cyc); ihit2++){
	    ...
	    if( inghitsum -> pln  == inghitsum2  -> pln  &&
	      ...
	      inghitsum2 -> lope  += tpe;
	      //inghitsum2 -> cyc   = -10;
	      if( (inghitsum -> time) < (inghitsum2 -> time) ) inghitsum2  -> time = inghitsum -> time;
	      SameHit=true;
	    }
	  }
	  if(!SameHit){
