...
const int MODE=1;
const int NXS = 20;
string NameXSParam[NXS] =        {"Fermi momentum $^{12}$C",
				  "Fermi momentum $^{16}$O",
				  "Binding energy $^{12}$C",
				  "Binding energy $^{16}$O",
				  "MEC norm. $^{12}$C",
...
inline bool IsReducedXSSyst(int s){ // for model. These corresponds to model tuning (ex: from RFG to SF)
...
void Evaluate1DError(TH2D * NominalMC, double **** CovarianceMatrix, TH1D * NominalMC_RecMom, TH1D * NominalMC_RecAngle){
  ...
  cout<<"Number of momentum bin = "<<NBinsMom<<", angle bin = "<<NBinsAngle<<endl;
...
#ifdef DEBUG
	//cout<<"bin "<<e0<<","<<e1<<","<<e0<<","<<f1<<" = "<<CovarianceMatrix[e0][e1][e0][f1]<<endl;
#endif
      }
    }
...
int main(int argc, char ** argv){
  ...
  int c=-1;
  while ((c = getopt(argc, argv, "o:wWrpmtnF:i:Ib")) != -1) {
    switch(c){
    case 'o':
      OutputName=optarg;
      break;      
  ...
  bool merged=true;
  char DetName[16];sprintf(DetName,(PM?"PM":"WM"));
  ...
  double ** MCReconstructedEvents_h2o = new double*[NBinsRecMom];
  double ** MCReconstructedEvents_ch = new double*[NBinsRecMom];
  double ** MCTrueEvents_h2o = new double*[NBinsRecMom];
  double ** MCTrueEvents_ch = new double*[NBinsRecMom];
  ...
  for(int i=0;i<NBinsRecMom;i++){
    ...
    MCReconstructedEvents_h2o[i] = new double [NBinsRecAngle];
    MCReconstructedEvents_ch[i] = new double [NBinsRecAngle];
    MCTrueEvents[i] = new double [NBinsRecAngle];
    MCTrueEvents_h2o[i] = new double [NBinsRecAngle];
    MCTrueEvents_ch[i] = new double [NBinsRecAngle];
    for(int j=0;j<NBinsRecAngle;j++){
      DataReconstructedEvents[i][j] = 0;
      MCReconstructedEvents[i][j] = 0;
      MCReconstructedEvents_h2o[i][j] = 0;
      MCReconstructedEvents_ch[i][j] = 0;
      MCTrueEvents[i][j] = 0;
      MCTrueEvents_h2o[i][j] = 0;
      MCTrueEvents_ch[i][j] = 0;
    }
  }
  ...
  for(int iter=iIter;iter<fIter;iter++){
    ...
    memset( CovarianceReducedTargets, 0, (EndError+1)*(NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    double CovarianceStatistics[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CovarianceStatistics, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CovarianceFlux[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CovarianceFlux, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CovarianceBirksPlus[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CovarianceBirksPlus, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CovarianceBirksMinus[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CovarianceBirksMinus, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CovarianceTotal[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CovarianceTotal, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );


    double CovarianceStatisticsTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};
    memset( CovarianceStatisticsTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    double CovarianceFluxTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};
    memset( CovarianceFluxTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    double CovarianceBirksPlusTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};
    memset( CovarianceBirksPlusTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    double CovarianceBirksMinusTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};
    memset( CovarianceBirksMinusTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    double CovarianceTotalTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};
    memset( CovarianceTotalTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );

    double CorrelationReduced[EndError+1][NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{{0}}}}};
    memset( CorrelationReduced, 0, (EndError+1)*(NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CorrelationStatistics[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CorrelationStatistics, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );
    double CorrelationFlux[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle];//={{{{0}}}};
    memset( CorrelationFlux, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*sizeof(double) );


    double **** CovarianceXS;
    double CovarianceXSTargets[NBinsMom][NBinsAngle][NBinsMom][NBinsAngle][2][2];//={{{{{{0}}}}}};;
    memset( CovarianceXSTargets, 0, (NBinsMom)*(NBinsAngle)*(NBinsMom)*(NBinsAngle)*(2)*(2)*sizeof(double) );
    ...
    double RelativeErrorWM[Systematics_Detector_End+1]={0,0.8,0.05,2.,0,0,0,0.6,0.82,0.41,0.31,0.13,0,0.35,.60,0,0,0};
    double RelativeErrorPM[Systematics_Detector_End+1]={0,1.1,0.47,1.04,0,0,0,0.58,0.58,0,0.18,0.12,0,0.97,.27,0,0,0};
    ...
    cout<<"###################################################"<<endl<<" Iter = "<<iter<<NBinsMom<<", "<<NBinsAngle<<endl;
    for(int e1=0;e1<NBinsAngle;e1++) sliceNominalSelectedMC_RecMom[e1] = new TH1D(Form("sliceNominalSelectedMC_RecMom[%d]",e1),Form("%d #circ < #theta_{#mu} < %d #circ",BinningAngle[e1],BinningAngle[e1+1]),NBinsMom,BinningMom);
	...
	for(int c0=0;c0<NBinsTrueMom;c0++){//loop over effect 0
      ...
      //if (ErrorType > 1 && ErrorType < 18) continue;
      //if (ErrorType > 1 && ErrorType < 18 && ErrorType != 3 && ErrorType != 4 && ErrorType != 5 && ErrorType != 6 ) continue;
      if (ErrorType == 19) continue;
	...
	continue;
      ...
      if(useKogaDetError && !IsReadySyst(ErrorType)){
	cout<<"Use CC inclusive errors: "<<useKogaDetError<<", is ready: "<<IsReadySyst(ErrorType)<<endl;
	double RelativeError=(PM?RelativeErrorPM[ErrorType]:RelativeErrorWM[ErrorType])/100.;
	for(int e0=0;e0<NBinsMom;e0++){
	  for(int e1=0;e1<NBinsAngle;e1++){
	    //MC = SIGNAL Nominal MC - BKG Varied MC. Note that SIGNAL Nominal MC = SELECTED Nominal MC + BKG Nominal MC.  
	...
	sprintf(txtDataName,"%s/XS/files/DataSelected_%s_Systematics%d_%d.txt",cINSTALLREPOSITORY,DetName,ErrorType,n);

	if(SideBand){
	  sprintf(txtMCNameSB,"%s/XS/files/MCSideBand_%s_Systematics%d_%d.txt",cINSTALLREPOSITORY,DetName,ErrorType,n);
	  sprintf(txtDataNameSB,"%s/XS/files/DataSideBand_%s_Systematics%d_%d.txt",cINSTALLREPOSITORY,DetName,ErrorType,n);
	}

	//2. Load files
	if(SideBand){
	  if(MC) XS->Xsec::LoadInputFilesSB(txtMCName,txtMCName,txtMCNameSB,txtMCNameSB,MCReconstructedEvents_TrueSignal,MCReconstructedEvents,MCReconstructedEvents,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	  else XS->Xsec::LoadInputFilesSB(txtDataName,txtMCName,txtDataNameSB,txtMCNameSB,MCReconstructedEvents_TrueSignal,DataReconstructedEvents,MCReconstructedEvents,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	}
	else{
	  if(MC) XS->Xsec::LoadInputFiles(txtMCName,txtMCName,MCReconstructedEvents_TrueSignal,MCReconstructedEvents,MCReconstructedEvents,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	...
	//3. Perform background substraction in the case of selected data (already done in the case of unfolded data)
	for(int e0=0;e0<NBinsMom;e0++){//loop over effect 0
	  for(int e1=0;e1<NBinsAngle;e1++){//loop over effect 1
	...
	if(water || ratio){
	  ...
	  XS->Xsec::LoadInputFiles_OnlyUnfoldedData((MC? txtMCName : txtDataName),MCReconstructedEvents_h2o,MCTrueEvents_h2o);
	...
	if(water || ratio){
	  if(SideBand){
	    XS->Xsec::LoadInputFilesSB((MC? txtMCName : txtDataName),txtMCName,txtMCNameSB,txtMCNameSB,MCReconstructedEvents_TrueSignal,(MC? MCReconstructedEvents_h2o:DataReconstructedEvents_h20),MCReconstructedEvents_h2o,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	    XS->Xsec::LoadInputFilesSB((MC? txtMCName : txtDataName),txtMCName,txtMCNameSB,txtMCNameSB,MCReconstructedEvents_TrueSignal,(MC? MCReconstructedEvents_ch:DataReconstructedEvents_ch),MCReconstructedEvents_ch,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	  }

	}
	  else{
	    */
	    if(SideBand) XS->Xsec::LoadInputFilesSB((MC? txtMCName : txtDataName),txtMCName,txtMCNameSB,txtMCNameSB,MCReconstructedEvents_TrueSignal,(MC? MCReconstructedEvents:DataReconstructedEvents),MCReconstructedEvents,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	    else XS->Xsec::LoadInputFiles((MC? txtMCName : txtDataName),txtMCName,MCReconstructedEvents_TrueSignal,(MC? MCReconstructedEvents:DataReconstructedEvents),MCReconstructedEvents,MCReconstructedBkgEvents,Efficiency,NumberOfPOT);
	...
	for(int e0=0;e0<NBinsMom;e0++){//loop over effect 0
	  for(int e1=0;e1<NBinsAngle;e1++){//loop over effect 1
	    ...
	    if(ErrorType==0){
#ifdef DEBUG
	      cout<<"bin "<<e0<<","<<e1<<" = "<<MCReconstructedEvents[e0][e1]<<endl;
#endif
...
#ifdef DEBUG
	      cout<<"Det error 5: N = "<<n<<", bin "<<e0<<","<<e1<<" = "<<MCReconstructedEvents[e0][e1]<<", relative = "<<RelativeValue<<endl;
#endif
    ...
    */
    ...
    if(UseTrashBins) NBins=(NBinsAngle-NTrashBinsAngle)*(NBinsMom-NTrashBinsMom);
	...
	for(int e1=0;e1<NBinsAngle;e1++){//loop over effect 1
	  ...
	  //cout<<e0<<" "<<e1<<" "<<Value<<" "<<Error<<" "<<ErrorSquared<<endl;      
	  cout<<e0<<" "<<e1<<" "<<Value<<" "<<Value*RelativeError<<" "<<ErrorTotalXS_Plus[e0][e1]<<endl;      
	...
	boxsliceErrorStat_RecMom[e0][e1]->Draw("same");
      ...
      sliceNominalSelectedMC_RecMom[e1]->GetXaxis()->SetRangeUser(0,5);
      //csliceRecMom[e1]->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecMom_slice%d.pdf",DetName,e1));
      //csliceRecMom[e1]->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecMom_slice%d.pdf",(ratio?"Ratio":DetName),e1));
    }
	...
	boxsliceErrorStat_RecAngle[e0][e1]->Draw("same");
      ...
      sliceNominalSelectedMC_RecAngle[e0]->GetXaxis()->SetRangeUser(0,60);
      //csliceRecAngle[e0]->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecAngle_slice%d.pdf",DetName,e0));
      //csliceRecAngle[e0]->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecAngle_slice%d.pdf",(ratio?"Ratio":DetName),e0));
    }
    ...
    NominalSelectedMC_RecMom->GetXaxis()->SetRangeUser(0,5);
    //cRecMom->Update();
    //cRecMom->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecMom.pdf",DetName));
    //cRecMom->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecMom.pdf",(ratio?"Ratio":DetName)));
    ...
    NominalSelectedMC_RecAngle->GetXaxis()->SetRangeUser(0,60);
    //cRecAngle->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecAngle.pdf",DetName));
    //cRecAngle->SaveAs(Form("plots/XSResults/Canvas_%s_Nominal_RecAngle.pdf",(ratio?"Ratio":DetName)));
    ...
    cout<<setprecision(2)<<std::scientific<<power*TotalValue<<"+-"<<std::fixed<<100*TMath::Abs(TotalRelErrorFlux)<<"%(Flux)+-"<<100*TMath::Abs(TotalRelErrorXS)<<"%(XS)+-"<<100*TMath::Abs(TotalRelErrorDet)<<"%(Det)+-"<<100*TMath::Abs(TotalRelErrorStat)<<"%(stat)"<<endl;
	...
	cout<<setprecision(2)<<NameXSParam[i]<<" & "<<100*TMath::Abs(TotalRelxsError[i])<<"\\% \\\\"<<endl;
      if(CorrelateMEC) cout<<"MEC-C and -O are treated as "<<(int)100*MEC_C_O_corr<<"\\% correlated in error #4"<<endl;
    }
    //#########################################END ERROR TABLE####################################
    TCanvas * cNominalSelectedMC = new TCanvas("cNominalSelectedMC");
    TFrame * fra = (TFrame*) gPad->GetFrame();
    fra->SetFillColor(kCyan);
    NominalSelectedMC->GetXaxis()->SetRangeUser(0,5);
    NominalSelectedMC->GetXaxis()->SetTitle("p_{#mu} (GeV/c)");
    NominalSelectedMC->GetXaxis()->SetTitleOffset(1.6);
    NominalSelectedMC->GetYaxis()->SetRangeUser(0,60);
    NominalSelectedMC->GetYaxis()->SetTitleOffset(1.6);
    NominalSelectedMC->GetYaxis()->SetTitle("#theta_{#mu} (#circ)");
    NominalSelectedMC->GetZaxis()->SetTitle("#frac{d^{2}#sigma}{dp_{#mu} d#theta_{#mu}} (cm^{2}/nucleon/GeV/#circ)");
    NominalSelectedMC->Draw("LEGO1");
    fra = (TFrame*) gPad->GetFrame();
    fra->SetFillColor(kCyan);
    gPad->Update();
    cNominalSelectedMC->Update();
    cNominalSelectedMC->SaveAs(Form("plots/XSResults/NominalSelectedMC_%s.pdf",DetName));
    //cNominalSelectedMC->SaveAs(Form("plots/XSResults/NominalSelectedMC_%s.pdf",(ratio?"Ratio":DetName)));
  ...
  //TFile * globalout=new TFile(Form("chi2_FD%d_%s.root",FakeDataSet,(ratio?"Ratio":DetName)),"recreate");
  globalout->cd();
