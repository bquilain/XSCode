...
int main(int argc, char ** argv){
  ...
  int c=-1;
  while ((c = getopt(argc, argv, "o:wp")) != -1) {
    switch(c){
    case 'o':
      OutputName=optarg;
      break;      
 ...
 //For Xsection errors
  ...
  for(int ErrorType=StartError;ErrorType<=EndError;ErrorType++){
    ...
    for(int n=0;n<NE[ErrorType];n++){
...
#ifdef RECONSTRUCTED
      //      sprintf(txtMCName,"${INSTALLREPOSITORY}/XS/files/MCSelected_Systematics%d_%d.txt",ErrorType,n);
      sprintf(txtMCName,"%s/XS/Selection1000_cutBkg%s.txt",cINSTALLREPOSITORY,DetName);
      XS->Xsec::LoadInputFiles_OnlySelectedData(txtMCName,MCReconstructedEvents);
      
      if(ErrorType>=7 && ErrorType<=Systematics_Detector_End) sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/DataSelected_Systematics%d_%d.txt",ErrorType,n);
      else if(ErrorType==0 && EndError>=7 && StartError<=Systematics_Detector_End) sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/DataSelected_Systematics%d_%d.txt",ErrorType,n); 
      else sprintf(txtDataName,"%s/XS/Selection1000_cutBkg%s.txt",cINSTALLREPOSITORY,DetName);
      //cout<<"good"<<endl;
      ...
      if(ErrorType>=7 && ErrorType<=Systematics_Detector_End) sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/DataUnfolded_Systematics%d_%d.root",ErrorType,n);
      else if(ErrorType==0 && EndError>=7 && StartError<=Systematics_Detector_End) sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/DataUnfolded_Systematics%d_%d.root",ErrorType,n); 
      else sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/DataUnfolded_Systematics0_0.root");
      //cout<<"good"<<endl;
      ...
      if(ErrorType>=7 && ErrorType<=Systematics_Detector_End) sprintf(txtMCName,"/home/bquilain/CC0pi_XS/XS/files/DataUnfolded_Systematics%d_%d.root",ErrorType,n);
      else sprintf(txtDataName,"/home/bquilain/CC0pi_XS/XS/files/MCUnfolded_Systematics0_0.root");
      //cout<<"good"<<endl;
      XS->Xsec::LoadInputFiles_UnfoldedData(txtMCName,DataReconstructedEvents,MCTrueEvents);
#endif
*/      
	...
	for(int e1=0;e1<NBinsAngle;e1++){//loop over effect 1
	  ...
	  for(int f0=0;f0<NBinsMom;f0++){//loop over effect 0
	    for(int f1=0;f1<NBinsAngle;f1++){//loop over effect 1
	      ...
	      CovarianceReduced[s1][e0][e1][f0][f1]+=(1./(NToysXsec-1.))*(NEventsXS[bin1]-NominalMC->GetBinContent(e0+1,e1+1))*(NEventsXS[bin2]-NominalMC->GetBinContent(f0+1,f1+1)); 
...
#ifdef XSTABLE
    cout<<"Error="<<Systematics_Xsec_Start+s1<<endl;
  ...
  for(int e0=0;e0<NBinsMom;e0++){//loop over effect 0
    for(int e1=0;e1<NBinsAngle;e1++){//loop over effect 1
      ...
      //      Error=TMath::Sqrt(ErrorSquared); ML 2017/03/09
      Error=ErrorSquared;
